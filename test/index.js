...
import lofF, {knn, kd, lrd} from "../src";
...
describe('testing lof', () => {
  it('should return lof and be a number with k of 2', () => {
    ...
    const results = dataset.map((item, index) => {
      const lof = lofF(2, coordinates, index);
      console.log(lof);
      ...
      return {
        ...
        "geometry": {
          "type": "Point",
          "coordinates": [...item.coordinates].reverse(),
        }
      };
    });
    ...
    fs.writeFileSync('mynewfile3.txt', JSON.stringify(results));
  });
});
